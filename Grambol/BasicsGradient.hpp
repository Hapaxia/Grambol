...
// Basics Gradient
//
// Copyright(c) 2024-2026 M.J.Silk
//
...
#ifndef GRAMBOL_BASICSGRADIENT_HPP
#define GRAMBOL_BASICSGRADIENT_HPP

#include "FullSymbol.hpp"
...
namespace Types
{
	namespace BasicGradient
	{
		...
		enum class Gradient
		{
			...
			Fit, // only for Frame, RoundedFrame and Parallelogram
		};

	} // namespace BasicGradient
} // namespace Types

inline sf::Color mixColors(const sf::Color& a, const sf::Color& b, const float ratio)
{
	const auto mixComponent = [](const std::uint8_t a, const std::uint8_t b, const float ratio)
	{
		const long int c{ std::lround((a * (1.f - ratio)) + (b * ratio)) };
		return static_cast<std::uint8_t>((c < 0) ? 0 : (c > 255) ? 255 : c);
	};
	return{ mixComponent(a.r, b.r, ratio), mixComponent(a.g, b.g, ratio), mixComponent(a.b, b.b, ratio), mixComponent(a.a, b.a, ratio) };
};

class BasicGradientBase : public FullSymbol
{
public:
	BasicGradientBase(sf::PrimitiveType primitiveType) : FullSymbol(primitiveType, { sf::Color::White, sf::Color::Black }), m_gradientType{ Types::BasicGradient::Gradient::Vertical } { }

	void setGradientType(Types::BasicGradient::Gradient gradientType) { m_gradientType = gradientType; priv_update(); }
	Types::BasicGradient::Gradient getGradientType() const { return m_gradientType; }

protected:
	Types::BasicGradient::Gradient m_gradientType;
};

template <Selection::BasicGradient>
class BasicGradient { BasicGradient() = delete; };

template <>
class BasicGradient<Selection::BasicGradient::Rectangle> : public BasicGradientBase
{
public:
	BasicGradient() : BasicGradientBase(sf::PrimitiveType::TriangleStrip) { }
...
template <>
class BasicGradient<Selection::BasicGradient::Ellipse> : public BasicGradientBase
{
public:
	BasicGradient() : BasicGradientBase(sf::PrimitiveType::TriangleFan), m_numberOfEdges(36u) { }
...
template <>
class BasicGradient<Selection::BasicGradient::Star> : public BasicGradientBase
{
public:
	BasicGradient() : BasicGradientBase(sf::PrimitiveType::TriangleFan), m_numberOfSpikes(5u), m_innerDistanceMultiplier(0.38196601125010515179541316563436f) { }
...
template <>
class BasicGradient<Selection::BasicGradient::Frame> : public BasicGradientBase
{
public:
	BasicGradient() : BasicGradientBase(sf::PrimitiveType::TriangleStrip), m_thickness(10.f) { }
...
template <>
class BasicGradient<Selection::BasicGradient::RoundedRectangle> : public BasicGradientBase
{
public:
	BasicGradient() : BasicGradientBase(sf::PrimitiveType::TriangleStrip), m_numberOfCornerEdges(16u), m_cornerRadius{ 10.f, 10.f } { }
...
template <>
class BasicGradient<Selection::BasicGradient::RoundedFrame> : public BasicGradientBase
{
public:
	BasicGradient() : BasicGradientBase(sf::PrimitiveType::TriangleStrip), m_numberOfCornerEdges(16u), m_thickness(10.f), m_outerCornerRadius{ 10.f, 10.f }, m_innerCornerRadius{ 5.f, 5.f } { }
...
template <>
class BasicGradient<Selection::BasicGradient::RegularPolygon> : public BasicGradient<Selection::BasicGradient::Ellipse>
{
public:
	BasicGradient() : BasicGradient<Selection::BasicGradient::Ellipse>() { BasicGradient<Selection::BasicGradient::Ellipse>::setNumberOfEdges(36u); }
...
template <>
class BasicGradient<Selection::BasicGradient::Parallelogram> : public BasicGradientBase
{
public:
	BasicGradient() : BasicGradientBase(sf::PrimitiveType::TriangleStrip), m_skew{ 0.1f } { }
...
inline sf::Vertex BasicGradient<Selection::BasicGradient::Rectangle>::priv_getVertex(const std::size_t vertexIndex) const
{
	sf::Vertex vertex;
	...
	{
	case 1u:
		vertex.position = { 1.f, 0.f };
		break;
	case 2u:
		vertex.position = { 0.f, 1.f };
		break;
	case 3u:
	...
	vertex.color = getColor(0u);

	switch (m_gradientType)
	{
	case Types::BasicGradient::Gradient::Horizontal:
		if (vertex.position.x > 0.5f)
			vertex.color = getColor(1u);
		break;
	case Types::BasicGradient::Gradient::Vertical:
	default:
...
inline sf::Vertex BasicGradient<Selection::BasicGradient::Ellipse>::priv_getVertex(const std::size_t vertexIndex) const
{
	sf::Vertex vertex;
	...
	else
	{
		...
		if (vertexIndex <= numberOfVerticesAroundPerimeter)
		{
			...
			vertex.position = { center.x + center.x * std::cos(radians), center.y + center.y * std::sin(radians) };
		}
		else
			vertex.position = { 1.f, center.y };
	}
	...
	{
	case Types::BasicGradient::Gradient::Horizontal:
		gradientAlpha = vertex.position.x;
		break;
	case Types::BasicGradient::Gradient::Vertical:
	default:
		gradientAlpha = vertex.position.y;
	}

	vertex.color = mixColors(getColor(0u), getColor(1u), gradientAlpha);

	return vertex;
}

inline sf::Vertex BasicGradient<Selection::BasicGradient::Star>::priv_getVertex(const std::size_t vertexIndex) const
{
	sf::Vertex vertex;
	...
	else
	{
		...
		else
		{
			...
			vertex.position = { center.x + center.x * radius * std::cos(radians), center.y - center.y * radius * std::sin(radians) };
		}
	}
...
inline sf::Vertex BasicGradient<Selection::BasicGradient::Frame>::priv_getVertex(const std::size_t vertexIndex) const
{
	sf::Vertex vertex;
	...
	const bool isOuterVertex{ vertexIndex % 2u == 0u };
	const unsigned int corner{ (vertexIndex / 2u) % 4u };
	...
	case 1u:
		if (isOuterVertex)
			...
			vertex.position = { 1.f - thickness.x, thickness.y };
		break;
	case 2u:
		if (isOuterVertex)
			vertex.position = { 1.f, 1.f };
		else
			vertex.position = { 1.f - thickness.x, 1.f - thickness.y };
		break;
	case 3u:
		if (isOuterVertex)
			vertex.position = { 0.f, 1.f };
		else
			vertex.position = { thickness.x, 1.f - thickness.y };
		break;
	case 0u:
	default:
		if (isOuterVertex)
			...
			vertex.position = thickness;
	}

	switch (m_gradientType)
	{
	case Types::BasicGradient::Gradient::Vertical:
		vertex.color = mixColors(getColor(0u), getColor(1u), vertex.position.y);
		break;
	case Types::BasicGradient::Gradient::Horizontal:
		vertex.color = mixColors(getColor(0u), getColor(1u), vertex.position.x);
		break;
	default:
	case Types::BasicGradient::Gradient::Fit:
		vertex.color = isOuterVertex ? getColor(0u) : getColor(1u);
		break;
	}
...
inline sf::Vertex BasicGradient<Selection::BasicGradient::RoundedRectangle>::priv_getVertex(const std::size_t vertexIndex) const
{
	sf::Vertex vertex;
...
inline sf::Vertex BasicGradient<Selection::BasicGradient::RoundedFrame>::priv_getVertex(const std::size_t vertexIndex) const
{
	sf::Vertex vertex;
	...
	{
	case Types::BasicGradient::Gradient::Vertical:
		vertex.color = mixColors(getColor(0u), getColor(1u), y);
		break;
	case Types::BasicGradient::Gradient::Horizontal:
		vertex.color = mixColors(getColor(0u), getColor(1u), x);
		break;
	default:
	case Types::BasicGradient::Gradient::Fit:
		vertex.color = isOuter ? getColor(0u) : getColor(1u);
		break;
	}
...
inline sf::Vertex BasicGradient<Selection::BasicGradient::Parallelogram>::priv_getVertex(const std::size_t vertexIndex) const
{
	sf::Vertex vertex;
	...
	if (m_skew < 0.f)
	{
		...
		{
		case 1u:
			vertex.position = { 1.f + m_skew, 0.f };
			break;
		case 2u:
			vertex.position = { -m_skew, 1.f };
			break;
		case 3u:
			vertex.position = { 1.f, 1.f };
			break;
	...
	else
	{
		...
		case 2u:
			...
			break;
		case 3u:
			vertex.position = { 1.f - m_skew, 1.f };
			break;
		case 0u:
		default:
			vertex.position = { m_skew, 0.f };
		}
	}
	...
	default:
	case Types::BasicGradient::Gradient::Fit:
		vertex.color = (vertexIndex % 2u == 0u) ? getColor(0u) : getColor(1u);
		break;
	}
...
#endif // GRAMBOL_BASICSGRADIENT_HPP
